...
def canny_edge_detection(img):
    ...
    G_angle = np.arctan2(Gy, Gx) * (180 / np.pi)  # 將角度從徑度轉換成度
    ...
    non_zero_gradients = np.extract(G_magnitude > 0, G_magnitude)
    if non_zero_gradients.size:
        sorted_gradients = np.sort(non_zero_gradients)
        n = int(len(sorted_gradients) * max_ratio)
        scale_value = 255 if n == 0 else sorted_gradients[-n]
        scale_ratio = 255 / scale_value
        G_magnitude = np.clip(G_magnitude * scale_ratio, 0, 255)
    ...
    # 將檢測到的強邊緣和弱邊緣分別設為指定的強度值
    res[strong_i, strong_j] = strong_val
    res[weak_i, weak_j] = weak_val
    
    # 用滯後閾值處理弱邊緣，強化與強邊緣相連的弱邊緣
    for i in range(1, M-1):
        for j in range(1, N-1):
            if (res[i, j] == weak_val):
                # 檢查周圍是否有強邊緣，如果有，則將當前弱邊緣轉化為強邊緣
...
def filter_img():
    ...
    mean_filtered = mean_filter(noisy_img, kernel_size = 5)

    median_filtered = median_filter(noisy_img, kernel_size = 5)
...
def canny_img():
    ...
    filtered_img = cv2.GaussianBlur(gray_img, (5, 5), 1.4)

    high_threshold_ratio = 0.2
    low_threshold_ratio = high_threshold_ratio * 0.5
    high_threshold = filtered_img.max() * high_threshold_ratio
    low_threshold = high_threshold * low_threshold_ratio

    canny_img = cv2.Canny(filtered_img, low_threshold, high_threshold)
